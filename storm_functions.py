...
def detect_storms(field, lon, lat, res, order, Npix_min, Npix_max, rel_amp_thresh, d_thresh, cyc, cut_lon, cut_lat, globe=False):
    '''
    ...
    field = spatial_filter(field, lon, lat, res, cut_lon, cut_lat)
    ...
    return lon_storms, lat_storms, amp_storms, area_storms

def detect_storms_Met(field, lon, lat, res, order, loc, Npix_min, Npix_max, rel_amp_thresh, d_thresh, cyc, cut_lon, cut_lat, globe=False):
    '''
    Detect storms in a way that mimics how an Operational Meteorologist would. ssh_crits every 4 mb
    '''
    llon, llat = np.meshgrid(lon, lat)
    ...
    ssh_crits = np.arange(92000,105000,400)
    if order == 'topdown':
        ssh_crits = np.flipud(ssh_crits)
    ...
    for ssh_crit in ssh_crits:
        ...
        for iregion in range(nregions):
            ...
            lon_ext = llon[exterior]
            lat_ext = llat[exterior]
            lon_int = llon[interior]
            lat_int = llat[interior]            
            area = region_Npix * res**2 * len_deg_lat() * len_deg_lon(np.mean(lat_int))
            if d_thresh < 1 :
                d_t = major_ecc(area, d_thresh)*2
            if d_thresh > 1 :
                d_t = d_thresh
            d = distance_matrix(lon_ext, lat_ext)
            is_small_storm = d.max() < d_t            

            # Detected storms:
            if (storm_area_within_limits * has_internal_ext * is_tall_storm * is_small_storm) :
                storm_object_with_mass = field * region
                # find centre of mass of storm
                if loc == 'center_of_mass':            
                    storm_object_with_mass[np.isnan(storm_object_with_mass)] = 0
                    j_cen, i_cen = ndimage.center_of_mass(storm_object_with_mass)
                    lon_cen = np.interp(i_cen, range(0,len(lon)), lon)
                    lat_cen = np.interp(j_cen, range(0,len(lat)), lat)
                # find min central pressure of storm
                if loc == 'min':
                    storm_object_with_mass[storm_object_with_mass==0] = np.nan
                    mn = np.nanmin(storm_object_with_mass)
                    j_cen, i_cen = np.argwhere(storm_object_with_mass == mn)[0]
                    lon_cen = np.interp(i_cen, range(0,len(lon)), lon)
                    lat_cen = np.interp(j_cen, range(0,len(lat)), lat)
                # Save storm
                lon_storms = np.append(lon_storms, lon_cen)
                lat_storms = np.append(lat_storms, lat_cen)
                # assign (and calculate) amplitude, area, and scale of storms
                amp_storms = np.append(amp_storms, amp_abs)
                area = region_Npix * res**2 * len_deg_lat() * len_deg_lon(lat_cen) # [km**2]
                area_storms = np.append(area_storms, area)
                # remove its interior pixels from further storm detection
                ...
                storm_mask[interior.astype(int)==1] = np.nan
                field = field * storm_mask
    return lon_storms, lat_storms, amp_storms, area_storms, field

def storms_list_c(lon_storms_c, lat_storms_c, amp_storms_c, area_storms_c):
    '''
    ...
    for ed in range(len(lon_storms_c)):
        storm_tmp = {}
        storm_tmp['lon'] = (lon_storms_c[ed])
        storm_tmp['lat'] = (lat_storms_c[ed])
        storm_tmp['amp'] = (amp_storms_c[ed])
        storm_tmp['area'] = (area_storms_c[ed])
        storm_tmp['type'] = list(repeat('cyclonic',len(lon_storms_c[ed])))
        storm_tmp['N'] = len(storm_tmp['lon'])
        storms.append(storm_tmp)

    return storms

def terrain_filter(storms, terrain, lat, lon, threshold):
    '''
    Filters out minima that are detected at a location with an elevation greater than the threshold.
    Storms is list of dictionaries of detected mins (ouput from storms_list)
    Terrain is a 2D array (lat lon) of locations higher than the threshold elevation 
    '''
    #make a lat, lon list of places that are above threshold
    lat_filt = lat[np.where(terrain>threshold)[0]] 
    lon_filt = lon[np.where(terrain>threshold)[1]] 
    filtered_storms = []
    #loop over all timesteps
    for tt in range(len(storms)):
        #round the lat and lons to the nearest 1/4 degree
        lat_tt = np.round(storms[tt]['lat']*4)/4
        lon_tt = np.round(storms[tt]['lon']*4)/4
        #loop over detected mins and determine which pass through filter
        passed = list(range(storms[tt]['N']))
        for ed in range(storms[tt]['N']):
            inds = np.where(lat_filt == lat_tt[ed])
            if len(np.where(lon_filt[inds] == lon_tt[ed])[0]) > 0 :
                passed.remove(ed)
        storm_tmp = {}
        storm_tmp['lon'] = (storms[tt]['lon'][passed])
        storm_tmp['lat'] = (storms[tt]['lat'][passed])
        storm_tmp['amp'] = (storms[tt]['amp'][passed])
        storm_tmp['area'] = (storms[tt]['area'][passed])
        storm_tmp['type'] = list(repeat('cyclonic',len(passed)))
        storm_tmp['N'] = len(passed)
        filtered_storms.append(storm_tmp)
